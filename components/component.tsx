...
import circuitUtil from '../circuits/utils/target/utils.json'
...
function Component() {
    ...
    const calculateProof = async () => {
        const calc = new Promise(async (resolve, reject) => {
            //calculate hash
            const result = await noirPedersen!.execute({ inputs: ["1"] });
            const proof = await noir!.generateFinalProof({ hash: result!.returnValue[0], turn: 1 });
    ...
    const initNoir = async () => {
        ...
        setBackend(backend);
        const backendPedersen = new BarretenbergBackend(circuitUtil);
        ...
        await toast.promise(noirPedersen.init(), {
            pending: 'Initializing Noir Utils...',
            success: 'Noir Utils initialized!',
            error: 'Error initializing Noir Utils',
        });
        setNoir(noir);
